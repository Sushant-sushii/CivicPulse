...
export default function Announcements({ user }) {
  ...
  const handleImageFileChange = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/announcements/upload-image`, {
        method: 'POST',
        headers: {
  ...
  const handlePublishSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('authToken') || localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/announcements/create`, {
        method: 'POST',
        headers: {
