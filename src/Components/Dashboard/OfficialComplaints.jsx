...
export default function OfficialComplaints({ user }) {
  ...
  const initDashboardFlow = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/complains?department=${encodeURIComponent(deptName)}&limit=1000`, {
        method: 'GET',
        headers: {
  ...
  const updateStatusOnServer = async (id, status, statement = '') => {
    try {
      const token = localStorage.getItem('authToken') || localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/complains/${id}/status`, {
        method: 'PUT',
        headers: {
